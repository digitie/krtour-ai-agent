...
본 문서는 `kor-travel-concierge` 프로젝트의 전체 시스템 설계와 구성 요소 간 데이터 흐름을 다룬다. 기준 문서는 Google Docs `AI유튜브여행_소형프로젝트_SpatiaLite_명세서`이며, 의사결정의 역사는 `decisions.md`의 ADR에서 별도로 관리한다.
...
- DB는 `kraddr_geo`와 분리된 `kor_travel_concierge`를 목표로 한다.
...
- `kor-travel-concierge`는 YouTube 장소 후보 provider가 되고, 범용
  `/api/v1/features/*` API를 제공한다. `kor-travel-map`은 이 API를 주기적으로
  pull하는 첫 consumer로서 후보를 feature로 승격한다.
- PinVi는 `kor-travel-map`이 만든 `feature_id`와 `feature_snapshot`을 자체
...
`kor-travel-concierge`는 1~2인이 개발·운영하고 동시 사용자가 10명 내외인 소형 프로젝트를 전제로 한다. 따라서 대규모 분산 크롤링보다 운영 단순성, 장애 원인 축소, 재현 가능한 로컬/단일 호스트 배포를 우선한다.
...
```
           ...
           │ - kor_travel_concierge DB│ │ Docker 서비스   │ │ - YouTube Data API v3 │
...
`kor_travel_concierge`를 기준으로 작성한다. RustFS는 애플리케이션 컨테이너에 내장하지
않고 별도의 로컬 Docker 서비스로 실행하며, 앱은 S3 호환 엔드포인트로 접근한다.
기본 host port는 고정값으로 API `http://localhost:12601`, MCP
`http://localhost:12602/mcp`, Web `http://localhost:12605`(host가 컨테이너 내부 API
...
배포는 `APP_ENV=production`+`API_KEYS`로 인증을 강제한다(ADR-24). 정적 `API_KEYS`와
`BACKEND_API_KEY`는 admin header credential로 취급하고, 외부 소비자는 DB에서 발급한
`read` 키를 사용한다. `read`는 명시된 장소·feature·theme·category 공급 GET만
허용하며 다른 모든 경로·method는 deny-by-default로 admin을 요구한다(ADR-36). 실제 무거운 작업
실행은 `scheduler`가 claim 방식으로 담당하여 API 서버와 MCP 서버가 직접 장시간
작업을 수행하지 않게 한다.

T-175는 이 scope capability와 관리 UI를 제공했고, T-176에서 production `kor-travel-map`의
Dagster·daemon에만 DB read key를 주입하는 운영 전환을 완료했다. Map API에는 consumer
인증 정보를 주입하지 않으며, 기존 consumer용 정적 admin entry는 새 BFF/operator admin
인증 정보로 교체한 뒤 제거했다. snapshot/changes 전체 순회와 실제 Dagster 가져오기 결과, read 공급
GET 200·write/내부 GET 403, 구 admin key 401을 n150에서 검증했다.

관리자 UI는 단일 관리자 계정으로 로그인한 뒤 httpOnly `SameSite=Strict` 세션 쿠키로
보호한다(ADR-32). 브라우저는 백엔드 관리자 API를 직접 호출하지 않고 same-origin
Next BFF만 호출한다. BFF는 세션을 검증한 뒤 서버 전용 `KTC_ADMIN_PROXY_SECRET`과
actor header를 주입하고, FastAPI는 trusted proxy peer CIDR과 shared secret이 모두
맞는 요청만 `/api/v1/admin/*`에 허용한다. 공개 REST API는 `X-API-Key` 헤더 또는
VWorld 호환 `?key=` query parameter를 검증한다. query credential은 DB `read` 키만
허용하고 admin·정적 키는 거부한다. Web UI에서 발급한 key는 DB에 hash·hint·`read|admin`
scope만 저장하며, 활성 `key_hash → scope` mapping을 짧은 TTL로 메모리 캐시한다. 생성·폐기
직후 cache를 무효화하고, scope 변경은 폐기 후 재발급으로만 수행한다. 무키 신뢰 CIDR 우회도
`read` 공급 경로로 제한한다.
...
- 전역 작업 상태는 `GET /api/v1/runs/queue` 한 번으로 사용자 `running`/`pending` 큐와 정확한 상태별 수·open attention 수를 읽는다. 응답 항목은 100건으로 제한하고 shell observer 하나만 10초 polling하며 수집·상태 화면은 같은 TanStack Query cache를 공유한다. 종료 이력은 `/runs?terminal=true&user_jobs_only=true` cursor 조회로 분리한다.
...
- 설정 모달에서 공개 API key의 `read|admin` scope를 선택해 생성하고 목록에서 scope를
  확인·폐기한다. 기본값은 `read`이며 생성된 key는 한 번만 평문으로 표시한다. 로그인
  시도·성공·실패·로그아웃 기록도 같은 화면에서 확인한다.

검수·작업·장소·테마 목록은 공통 `{items,next_cursor,has_more,total,newest_id,newer_than}`
envelope와 filter fingerprint가 묶인 keyset cursor를 사용한다. 첫 page의 `newest_id`를 ID
watermark로 cursor에 보존해 더 큰 ID의 새 행이 기존 순회에 끼어들지 않게 하고, 새 행 수는
`newer_than_id`의 실제 count로 계산한다. 단건 endpoint는 page 밖 대상도 직접 조회한다.
범용 feature 공급 API는 실소비 sequence cursor 계약을 그대로 유지한다. 상세 계약은
`docs/list-api-contract.md`가 정본이다.

검수 큐의 서버 조회는 `q`(후보명·위치 단서의 literal 부분 문자열), `sort`(`oldest|newest`),
`is_domestic`(`true|false|전체), `status`(`needs_review|ignored|removed`), 사유·출처·source provenance를
동일한 filter fingerprint에 포함한다. 화면은 FIFO 소진을 위해 `oldest`를 기본으로 쓰고 300건씩
append하되, snapshot이 열린 동안 행을 자동 재배치하지 않는다. 대신 60초 경량 probe가 watermark
뒤 신규 건수와 현재 filter의 정확한 `total` 변화를 확인하고 사용자가 명시적으로 snapshot을
다시 시작하게 한다. 검색·정렬·filter는 URL의 정본 상태라 새로고침·뒤로 가기·공유 링크에서
복원된다. 일반 행 선택은 URL 소음을 만들지 않으며, 명시적 `?candidate=` 딥링크로 진입한 경우에만
목록 page 밖 candidate도 단건 endpoint로 바로 열고 현재 filter 밖이라는 맥락을 함께 표시한다.
저장·제외·삭제 성공 시에는 진행 중인 page/probe 요청을 취소한 뒤 모든 관련 cache에서 처리된 ID를
제거하고 현재 scope를 다시 검증해, 늦은 응답이 처리 완료 행을 되살리지 못하게 한다.

`removed`는 제외(`ignored`)와 soft delete 이력을 합친 복구 전용 목록이다. 이 화면에서는 외부 장소
검색·AI 의견·VWorld 지도를 호출하지 않고, 상세 이력 확인과 `reopen`만 허용한다. 저장·제외·삭제가
성공하면 서버가 발급한 opaque descriptor를 마지막 1건 snackbar에 기한 없이 유지하고, 다음 단건
처리가 이를 대체한다. 복구는 후보를 `needs_review`로 되돌리되 영상의 `is_excluded`는 독립 정책으로
남긴다. forward 응답이 유실됐을 때는 exact detail의 상태·descriptor뿐 아니라 요청 전에 발급한
`client_operation_id`가 서버의 최신 검수 operation과 일치해야만 그 처리를 내 성공으로 승격한다.
이 operation 표식은 resolve core와 audit을 commit한 뒤 행정구역 보강까지 끝난 candidate revision과
연결 place revision에 묶는다. 기대 상태인데 표식만 아직 없는 finalization 구간은 짧고 제한된
...
멱등 키는 `audit_logs.idempotency_key`·`idempotency_state` 전용 컬럼과
`(actor_type, action, idempotency_key)` partial unique index로 조회한다. 외부 admin 보강을 포함하는
장소 보정·후보 확정은 core write와 `pending` 감사 행을 먼저 원자 commit한 뒤 owner/lease fencing으로
후처리하고, 최신 도메인 snapshot과 `final` 전이를 같은 transaction에 묶는다. 손상·만료 lease는 새
owner가 인계할 수 있지만 이전 owner의 늦은 응답은 장소와 감사 결과를 덮을 수 없다.
...
- `kor-travel-concierge`: 다운로드한 원본 동영상 또는 오디오 파일, 원본 자막, 자동 자막, `yt-dlp` 추출 자막, `faster-whisper` 전사 결과, 대표 프레임 JPEG를 함께 저장하는 단일 버킷
...
로컬 Docker 기준 RustFS 호스트 포트는 S3 API `12101`, 콘솔 `12105`를 사용한다. 호스트와 로컬 venv에서는 `http://127.0.0.1:12101`으로 접근하고, 공개 객체 URL은 `http://127.0.0.1:12101/kor-travel-concierge/{object_key}` 형식을 사용한다. 앱 컨테이너 내부에서는 외부 고정 RustFS 서비스를 `http://host.docker.internal:12101`로 접근한다. RustFS 컨테이너 자체는 공식 기본 포트인 S3 API `9000`, 콘솔 `9001`을 유지하되 이 repo Compose의 기본 서비스에는 포함하지 않는다. 상태 확인은 `/health/live`를 우선 사용한다.
...
- `place_id` (Integer, PK)
- `lifecycle_origin` (String) - `candidate_created`, `persistent`, `legacy_unknown`
- `origin_candidate_id` (Integer, FK, Nullable) - 후보 확정 과정에서 생성된 장소의 provenance
- `state_revision` (BigInteger) - 모든 UPDATE마다 DB trigger가 정확히 1 증가
- `name` (String)
...
- `deleted_at`, `deletion_reason`, `deleted_by` - hard delete를 대신하는 복구 가능 삭제 상태
- `provider_evidence_json.review.last_client_operation` - 최신 사용자 검수 요청의 UUID·action·시각과
  그 요청이 최종 확정한 candidate/place revision snapshot. 현재 revision과 다르면 API에는 operation
  ID를 노출하지 않는다.
- `created_at` (DateTime)

후보/장소 revision은 ORM이 직접 증가시키지 않고 PostgreSQL trigger가 단독 소유한다. forward
검수는 `expected_revision`, 복구는 후보 상태·revision과 연결 장소 revision을 담은 canonical
base64url descriptor로 stale 요청을 차단한다. 장소 생명주기 쓰기는
`lifecycle advisory(174) → feature export advisory(175) → candidate → place → mapping → asset`
순서를 공유한다. `candidate_created` 장소는 생성 후보의 소유물이 아니라 provenance다. 어느 후보가
마지막 참조를 제거하든 활성 후보 참조와 영상 매핑이 모두 0일 때만 정리하며, `persistent`와
`legacy_unknown`은 자동 정리하지 않는다. 장소를 지워도 `media_assets` 행과 RustFS 객체는 보존하고
장소 링크만 해제한다.
...
`raw_video`, `subtitle`, `transcript`, `frame`은 모두 `kor-travel-concierge` 버킷에 저장하고 `features/` prefix 아래의 객체 키로 구분한다. `MEDIA_RETENTION_POLICY`는 새 자산 행의 기본 보존 정책이며, 현재는 행 단위 `retention_policy`도 `infinite`로 기록해 RustFS 객체 lifecycle 만료 정책을 두지 않는 운영 계약을 보강한다.
...
`kor-travel-map`과 PinVi feature 연계 POI/curated plan에서 영상·유튜버·재생목록
...
- `youtube_video_analysis_runs`: transcript 기반 추출, YouTube URL Gemini 요약,
  ...
  남긴다. T-184 이후 `owner_crawl_run_id`, `owner_retry_count`, `claim_token`을
  함께 저장한다. parent heartbeat·retry generation과 token을 apply 직전 다시 잠가
  검증하므로 stale 재투입 전 worker의 늦은 성공·실패가 최신 run이나 canonical 결과를
  덮지 않는다. 같은 영상·종류의 중복 run은 먼저 완료된 결과를 정본으로 두고 나머지를
  `superseded` 사유의 terminal 실패로 남긴다.
...
외부 LLM을 기다리는 URL summary, reconcile, 영상 요약, deep research는 입력 snapshot을
확정한 뒤 DB transaction을 종료한다. 응답 적용 시 export advisory lock과 owner/revision
fence를 다시 잡아 입력 drift면 bounded 재시도하고, 사람 검수나 다른 canonical 결과가 먼저
확정됐으면 AI 결과를 정본에 적용하지 않는다. `review_candidate`와 auto-match audit이 소유한
사람 메모·판정은 reconcile이 덮지 않으며, AI 의견은 provider evidence에만 보존한다. 영상·채널·
재생목록 metadata, 영상 요약, 장소 주소·설명 변경은 연관된 모든 활성 후보를 durable dirty로
표시해 incremental feature export와 전량 sync가 같은 payload에 수렴한다.
...
`video_place_mappings.provider_evidence_json`에도 복사한다. **사용자 결정(2026-08-13)**으로
검수자가 명시적으로 선택한 Google Places 결과는 다른 provider와 같이 지도 선택과
`travel_places`·review resolution provenance 저장에 사용할 수 있다. 이 예외는 검수
선택·확정 경로에 한정하며 Google/provider 지오코딩 응답 cache와 외부 feature export는
별도 정책 경계를 유지한다.
...
- `payload_hash`: `kor-travel-map`이 `SourceRecord.raw_payload_hash`로 사용할 값.
...
추가한다. 특정 consumer 이름을 REST path에 넣지 않으며, 외부 호출은 ADR-36의 DB `read`
키를 `X-API-Key`로 전달한다. 정적 admin 키를 consumer에 배포하지 않는다.

계약 정본은 `docs/feature-export-api.md`다. T-068/T-069 기준 PinVi 소비는
`kor-travel-concierge` DB 직접 접근이나 자동 등록이 아니라, `kor-travel-map`이 만든
...
검수 undo는 전역 상태 라이브러리 대신 검수 화면 소유의 단일 slot으로 관리한다. mutation 시작
snapshot, queue scope, workflow epoch, 상세 modal generation을 함께 고정해 필터 전환·A→B→A 선택·
늦은 응답이 현재 후보를 덮지 못하게 한다. 모바일 상세 처리 성공은 operation ID와 생성 시각을 포함한
descriptor를 검수 목록으로 session handoff한다. 목록은 값을 읽기 전에 먼저 제거하고 5분이 지난
handoff는 폐기한다. 복구 전용 상태에서는 외부 provider query를 mount하지 않는다.
